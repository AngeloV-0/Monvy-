...
function renderizarListaInicio(){
  ...
  el.innerHTML=recentes.map(m=>`
    <div class="mov-item" onclick="abrirModalEditar('${m.id}')">
      ${iconeHtml(m.categoria, m.tipo, 32)}
      <div class="mov-info">
...
function atualizarTelaCategorias(){
  ...
  grid.innerHTML=sorted.map(([cat,val])=>{
    ...
    const catPng=getIconeCat(cat);
    return `<div class="categoria-card"><div class="cat-info">${catPng?`<img src="${catPng}" style="width:22px;height:22px;object-fit:contain;flex-shrink:0">`:''}<span class="cat-nome">${cat}</span><span class="cat-val red">${fmt(val)}</span></div><div class="cat-bar-wrap"><div class="cat-bar" style="width:${pct}%"></div></div><span class="cat-pct">${pct}%</span></div>`;
  }).join('');
...
function gerarInsights(){
  ...
  if(sai>ent*0.9&&ent>0)
    insights.push({png:'icone-urgente.png',bg:'rgba(239,68,68,0.15)',titulo:'Gastos altos',desc:`Seus gastos representam ${((sai/ent)*100).toFixed(0)}% das entradas. Considere reduzir.`,acao:null});
  if(dividas.length>0)
    insights.push({png:'icone-cartao-02.png',bg:'rgba(239,68,68,0.12)',titulo:'Dívidas ativas',desc:`Você tem ${dividas.length} dívida(s). Veja a aba Dívidas.`,acao:()=>irPara('dividas')});
  const cVenc=contas.filter(c=>!c.paga&&c.vencimento<dataHoje());
  if(cVenc.length>0)
    insights.push({png:'icone-urgente.png',bg:'rgba(245,158,11,0.15)',titulo:'Contas vencidas',desc:`${cVenc.length} conta(s) passaram do vencimento.`,acao:()=>irPara('contas')});
  const mPerto=metas.filter(m=>m.valor>0&&(m.atual/m.valor)>=0.8);
  if(mPerto.length>0)
    insights.push({png:'icone-trofeu.png',bg:'rgba(34,197,94,0.12)',titulo:'Meta quase atingida!',desc:`${mPerto[0].nome} está em ${((mPerto[0].atual/mPerto[0].valor)*100).toFixed(0)}%.`,acao:()=>irPara('metas')});
  ...
  if(cEl)cEl.textContent=`${insights.length} insight(s)`;
  list.innerHTML=insights.map(i=>`
    <div class="insight-item" style="cursor:${i.acao?'pointer':'default'}" onclick="${i.acao?'('+i.acao.toString()+')()':'void 0'}">
      <div style="width:44px;height:44px;border-radius:12px;background:${i.bg};display:flex;align-items:center;justify-content:center;flex-shrink:0">
        <img src="${i.png}" alt="${i.titulo}" style="width:26px;height:26px;object-fit:contain">
      </div>
      <div>
        <div style="font-weight:700;font-size:.88rem;color:var(--white)">${i.titulo}</div>
        <div style="font-size:.78rem;color:var(--gray);line-height:1.4;margin-top:2px">${i.desc}</div>
      </div>
    </div>`).join('');
}
