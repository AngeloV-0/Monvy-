...
async function carregarAtivosB3() {
  ...
  try {
    ...
    const res = await fetch(`https://brapi.dev/api/quote/${tickers}?fundamental=false&logourl=true`);
...
async function carregarGraficoB3(symbol, range, interval) {
  ...
  try {
    const url = `https://brapi.dev/api/quote/${symbol}?range=${range}&interval=${interval}&fundamental=false`;
...
async function buscarAtivoB3() {
  ...
  try {
    const r = await fetch(`https://brapi.dev/api/quote/${input}?fundamental=false&logourl=true`);
